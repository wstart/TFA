...
- **🧪 实验室（Lab）**：实验功能的容器，目前包含：
  ...
  - **PTY / 进程监控**：pty 用量 / 上限、TFA 的 `tmux -CC` 客户端与孤儿数、残留探针 socket，
    并可一键清理孤儿客户端释放 pty。
  - **办公室**：把每个会话渲染成一个**像素小人**（美术来自 [pixel-agents](https://github.com/pixel-agents-hq/pixel-agents)，
    MIT），按分组分区——**在输出**的坐办公桌打字（头顶显示最新输出、脚下显示工作时长），**空闲**的在
    休息区自由走动；状态切换时走过去而非瞬移，可**拖小人改分组**、点选 / 双击进入终端 / 右键操作。Marvis 风亮色卡片。
...
- [x] **实验室** — 系统监控 / Token 用量（ccusage 移植）/ PTY · 进程监控 / 办公室（会话像素小人可视化）
